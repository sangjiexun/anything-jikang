  ...
  if (loading) {
    return (
      <div className="flex flex-col bg-pulse w-full h-10 items-center justify-center">
        <p className="text-xs text-white animate-pulse">{t("thread.loading")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col" role="list" aria-label="Threads">
      <ThreadItem
        ...
        thread={{ slug: null, name: t("thread.default") }}
...
function NewThreadButton({ workspace }) {
  ...
  const onClick = async () => {
    ...
    if (!!error) {
      showToast(`${t("thread.could_not_create")} - ${error}`, "error", { clear: true });
      setLoading(false);
      return;
    }
